...
for e in range(n_experiments):
    ...
    ts_learner1 = SWTS_Learner(n_arms=n_arms, window_size=window_size)
    ts_learner2 = SWTS_Learner(n_arms=n_arms, window_size=window_size)
    learners.append(ts_learner1)
    ...
    for i in range(0,T):
        ...
        ts_learner1 = get_learner(age, gender, learners, split_age, split_gender)
        if ((i % 7) == 0 and i != 0):
            if (check_split_on_age):
                split_age = split_on_age(z)
                if (split_age):
                    learner_under = ts_learner1
                    learner_over = ts_learner1
            ...
            if(check_split_on_gender):
                ...
                if (split_gender):
                    learner_under_men = ts_learner1
                    learner_under_women = ts_learner1
                    learners.append(learner_under_men)
                    learners.append(learner_under_women)
                    check_split_on_gender = False

        #Thomposon Sampling Learner
        pulled_arm = ts_learner1.pull_arm()
        reward = generate_reward_phase(age,gender,p,pulled_arm,current_phase)
        ts_learner1.update(pulled_arm, reward)

        z = z.append({'Gender' : gender, 'Age': age,'Arm':pulled_arm,'Reward':reward},ignore_index=True)

        #Greedy Learner
        pulled_arm = ts_learner2.pull_arm()
        #reward = env.round(pulled_arm)
        reward = generate_reward_phase(age,gender,p,pulled_arm,current_phase)
        ts_learner2.update(pulled_arm, reward)

    ts_rewards_per_experiment.append(ts_learner1.collected_rewards)
    gr_rewards_per_experiment.append(ts_learner2.collected_rewards)
